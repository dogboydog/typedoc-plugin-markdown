...
export function getReflectionType(model: DeclarationReflection, collapse: Collapse) {
  if (model.signatures) {
  ...
  if (model.indexSignature || model.children) {
    ...
    if (declarationIndexSignature) {
      ...
      const obj = declarationIndexSignature.type
        ? typeTemplate(declarationIndexSignature.type as allowedTypes)
        : '`unknown`';
...
export function getFunctionType(modelSignatures: SignatureReflection[]) {
  const functions = modelSignatures.map((fn) => {
    ...
    const params = fn.parameters
      ...
      : [];
    const returns = fn.type ? typeTemplate(fn.type) : '`unknown`';
